...
@ContextConfiguration(classes = { AppConfig.class, MongoDBConfiguration.class,
...
public class DBliveryStatisticsTestCase {
	...
	@Autowired
	private DBliveryServiceImpl service;
	...
	public void prepareDB() throws Exception {
		...
		this.client.getDatabase("bd2_grupo" + this.getGroupNumber()).getCollection("orders")
				.createIndex(Indexes.geo2dsphere("position"));
	}
	...
	public void testGetTopNSuppliers() {
		...
		this.assertListEquality(suppliers.stream().map(property -> property.getName()).collect(Collectors.toList()),
				Arrays.asList("La Trattoria", "Olivia Pizzas & Empanadas", "El Ladrillo", "Pizza Nova"));
	}
	...
	public void testGetSoldProductsOn() throws ParseException {
		...
		this.assertListEquality(products.stream().map(property -> property.getName()).collect(Collectors.toList()),
				Arrays.asList("Filet de merluza a la romana", "Bife de chorizo grillado", "Milanesa americana",
						"Ensalada de hojas verdes y queso"));
	}
